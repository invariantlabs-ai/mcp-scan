...
def print_scan_path_result(result: ScanPathResult, print_errors: bool = False) -> None:
    if result.error is not None:
        rich.print(format_path_line(result.path, result.error.status))
        if print_errors and result.error.traceback is not None:
            console = rich.console.Console()
            console.print(result.error.traceback)
        return
    ...
    for server in result.servers:
        if server.error is not None:
            server_print = path_print_tree.add(format_servers_line(server.name or "", server.error.status))
            if server.error.traceback is not None:
                server_tracebacks.append((server, server.error.traceback))
        else:
